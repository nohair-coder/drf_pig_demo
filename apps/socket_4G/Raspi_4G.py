...
from learn.CAN import UsrCAN, CAN_Analysis
...
def Init_4GSocket():
    ...
    CANRecvThread = threading.Thread(target=Recv_4G, args=(socket_server, CANaddr))
    CANSendThread = threading.Thread(target=Send_4G, args=(socket_server, CANaddr))
    CANRecvThread.start()
    CANSendThread.start()
    CANRecvThread.join()
    CANSendThread.join()
...
def Send_4G(socket_server, CANaddr):
    'CAN发送'
...
def Recv_4G(socket_server, CANaddr):
    'CAN接收'
