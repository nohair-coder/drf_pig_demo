...
def CANRecv():
    ...
    while exit_flag != True:
        try:
            ...
            if databyte != None:
                msg = UsrCAN.Message(arbitration_id=struct.unpack('i', databyte[4:0:-1]),
                                     is_remote_frame=databyte[0] & 0x40, extended_id=databyte[0] & 0x80,
                                     dlc=databyte[0] & 0x0f, data=databyte[5:13])
